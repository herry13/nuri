...
class Sfp::Module::Hadoop2Master < Sfp::Module::TarPackage
	...
	def configure(p={})
		...
		dir = File.dirname(__FILE__) + '/hadoop2'
	...
	def installed?
		(super and ::File.exist?(@model['data_dir']))
	end
	...
	def add_user_group
		...
		if `grep '^#{user}' /etc/passwd`.length <= 0
			password = @model['password']  # TODO -- disable login if password is empty
			home = @model['home']
	...
	def create_data_directory
		...
		[map['namenode_name_dir'], map['datanode_data_dir'], map['yarn_local_dir'], map['yarn_log_dir']].each do |dir|
			...
			Dir.mkdir(dir)
			shell "chown -R #{@model['user']}:#{@model['user']} #{dir}"
		end
