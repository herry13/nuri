...
module Nuri
	module Module
		class Nginx
			...
			def enable_php_mysql_module
				return (
					Nuri::Helper::Package.install('php5-mysql') and
					Nuri::Helper::Service.stop('php5-fpm') and
					Nuri::Helper::Service.start('php5-fpm')
				)
			end

			def disable_php_mysql_module
				return (
					Nuri::Helper::Package.uninstall('php5-mysql') and
					Nuri::Helper::Service.stop('php5-fpm') and
					Nuri::Helper::Service.start('php5-fpm')
				)
			end
			...
			def save_config(config=nil)
				...
				default = "# DO NOT MODIFY (auto-generated by Nuri)
server {
   listen #{config['port']};
...
"
				if config['php_module']
					release = Nuri::Helper::Command.getoutput(". /etc/lsb-release; echo $DISTRIB_RELEASE").to_f
					php_fpm = (release <= 12.04 ? "127.0.0.1:9000" :
